...
In the benchmarks of [our preprint](https://doi.org/10.64898/2026.03.17.712436), 90% of the CAMA-C-2 reads are used for assembly, and the remaining 10% are used **only** for assembly evaluation. Although **you do not need to perform this step in your own assembly project**, we will carry out read subsampling here using [seqkit](https://github.com/shenwei356/seqkit) so that the results of this tutorial match those reported in our preprint.
...
Once the clean contigs and synref are ready, we can proceed with contig error correction and scaffolding using `nohic-asm.sh`. The general usage of this sub-script is as follows (please read [our preprint](https://doi.org/10.64898/2026.03.17.712436) to understand the options for contig correction).
